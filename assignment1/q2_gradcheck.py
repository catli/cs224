...
def gradcheck_naive(f, x):
    ...
    while not it.finished:
        ...
        xminus[ix] = xminus[ix] - h
        
        random.setstate(rndstate)
        fxplus, gradplus = f(xplus)
        
        random.setstate(rndstate)
        fxminus, gradminus = f(xminus)
        numgrad = np.divide((fxplus - fxminus), h*2)
        # print("****numgrad*******")
        # print(numgrad)
        ### END YOUR CODE
        # print "Checking..."
...
def your_sanity_checks():
    ...
    """
    quad = lambda x: (np.sum(x ** 2), np.sum(x * 2))
    print "Running your sanity checks..."
    ### YOUR CODE HERE
    gradcheck_naive(quad, np.array([0.4869949,-0.38385514,-1.78016354]))
    ### END YOUR CODE
